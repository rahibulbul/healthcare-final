...
export const FetchEmployeeRoleById = async (roleId) => {
    ...
    if (roleSnapshot.exists()) {
        ...
        return {
            ...
            permissions: roleData.permissions  // Ensure permissions are stored here
        };
    } else {
...
export const UpdateEmployeeRole = async (roleId, updatedRoleData) => {
    ...
    // Add the roleId to the updatedRoleData object
    const roleWithId = { ...updatedRoleData, id: roleId };

    try {
        await set(roleRef, roleWithId);  // Update the role data including the roleId
